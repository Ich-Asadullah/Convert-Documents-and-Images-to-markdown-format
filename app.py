...
from antropic_analyzer import process_pdf as antropic_pdf_analyzer
from antropic_analyzer import process_image as antropic_image_analyzer
import asyncio
...
async def process_file_antropic(file, file_type):
    if file_type == "pdf":
        results, total_cost, inp_tokens, res_tokens = await antropic_pdf_analyzer(file)
        st.markdown("\n".join(results))
    else:  # image
        result, total_cost, inp_tokens, res_tokens = await antropic_image_analyzer(file)
        st.markdown(result)
    st.write(f"Total cost: ${total_cost:.6f}")
...
def main():
    ...
    analyzer_option = st.radio("Select Analyzer", ("Azure", "OpenAI", "Antropic Image"))
    ...
    if uploaded_file is not None:
        ...
        file_type = "pdf" if uploaded_file.type == "application/pdf" else f"image/{uploaded_file.name.split('.')[-1]}"
        
        if st.button("Analyze Document"):
            with st.spinner("Analyzing document..."):
                if analyzer_option == "Azure":
                    process_file_azure(uploaded_file, file_type)
                elif analyzer_option == "OpenAI":  # OpenAI
                    asyncio.run(process_file_openai(uploaded_file, file_type))
                else:
                    asyncio.run(process_file_antropic(uploaded_file, file_type))
